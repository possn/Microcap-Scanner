...
#
# Copy/paste this whole file and commit. No further edits required.
...
# Liquidity filters (micro/smallcap practical defaults)
...
# Thresholds tuned to produce *some* candidates most days, while keeping EXECUTAR strict
# Compression gate (AGUARDAR/QUASE eligibility):
BBZ_GATE = float(os.environ.get("BBZ_GATE", "-0.7"))       # was too strict previously
ATRPCTL_GATE = float(os.environ.get("ATRPCTL_GATE", "0.45"))

# Base gate (microcaps are messy; allow broader bases)
BASE_DD_MAX = float(os.environ.get("BASE_DD_MAX", "0.55"))          # max drawdown inside base
CONTRACTION_MAX = float(os.environ.get("CONTRACTION_MAX", "0.85"))  # base range vs prev range

# Dry-up gate (keep loose; microcaps often noisy)
DRYUP_MAX = float(os.environ.get("DRYUP_MAX", "0.95"))  # vol10/vol60 must be below this

# EXECUTAR trigger strictness
VOL_CONFIRM_MULT = float(os.environ.get("VOL_CONFIRM_MULT", "1.5"))
MAX_GAP_UP = float(os.environ.get("MAX_GAP_UP", "1.12"))  # block big gap opens vs prev close
...
# -----------------------------
# Holdings (iShares) robust CSV cleaning
# -----------------------------
...
def fetch_holdings_csv(url: str) -> pd.DataFrame:
    ...
    if header_idx is None:
        # If HTML is returned, first "column" looks like "<!doctype html>"
        # Fail explicitly so we can see it in errors.
        if "<!doctype html" in text.lower():
            raise RuntimeError("Holdings URL devolveu HTML (bloqueio/redirect).")
        # last attempt: let pandas try
...
def get_universe_from_holdings(url: str) -> list[str]:
    ...
    # Normalize to Yahoo-style for some sources; Stooq uses ".us" in URL, we use lower later
    tickers = [t.replace(".", "-").upper() for t in tickers]
    return tickers


# -----------------------------
# OHLCV (Stooq) with cache (repo workspace)
# -----------------------------
...
def fetch_ohlcv_stooq(ticker: str) -> pd.DataFrame:
    """
    Returns DataFrame with columns: date, open, high, low, close, volume (lowercase)
    Uses cache merge to reduce requests.
    """
    ensure_dirs()
    ...
    # Stooq expects symbol like "aapl.us" (we pass {symbol} already containing suffix in fmt)
    # If user provided OHLCV_URL_FMT with {symbol}.us we pass just ticker (without .US)
    # Here: take ticker without "-": keep "-" and replace with "." for stooq US tickers often work.
    sym = ticker.lower().replace("-", ".")
    url = OHLCV_FMT.format(symbol=sym)
    ...
    if not all(c in df.columns for c in need):
        raise RuntimeError(f"{ticker}: OHLCV formato inesperado: {df.columns.tolist()}")
...
# -----------------------------
# Base detection (simple + robust)
# -----------------------------
def base_scan(df: pd.DataFrame) -> tuple[bool, int, float, float, float, float]:
    """
    Returns (ok, win, high_base, low_base, dd, contraction_ratio)
    Windows: 20..80 sessions (4..16w)
    dd = (high - low) / high
    contraction_ratio = base_range / prev_range (prev=120 sessions before base)
    """
    if len(df) < 260:
    ...
    for win in [20, 25, 30, 35, 40, 50, 60, 70, 80]:
        ...
        # pass/fail with relaxed microcap bounds (configurable)
        if dd <= BASE_DD_MAX and contr <= CONTRACTION_MAX:
            # prefer tighter dd and lower contr
...
def main() -> None:
    ...
    if not universe_all:
        msg = [f"[{now}] Microcap scanner (Modo A) [BUILD=FIXED_V1]"]
        msg.append("ERRO: universo vazio (holdings falharam).")
        if errors:
            msg.append("")
            msg.append("ERROS (top 5):")
            for e in errors[:5]:
                msg.append(f"- {e}")
        tg_send("\n".join(msg))
        return

    # Preselect by simple sampling to avoid rate limits: evaluate up to MAX_TICKERS tickers
    tickers = universe_all[:MAX_TICKERS]

    # Diagnostics counters (computed reliably without needing you to place += everywhere)
    n_hist_ok = 0
    n_liq_ok = 0
    n_comp_ok = 0
    n_base_ok = 0
    ...
    results_exec = []
    results_watch = []
    results_near = []  # passed compression + liquidity but failed base/dry
    ...
    for i, t in enumerate(tickers):
        try:
            ...
            # Percentile of ATR% in last ~252 sessions
            ...
            # Compression gate (loose enough to get candidates)
            ...
            # Trigger logic for EXECUTAR (strict)
            ...
            # Keep a few representative errors (don’t flood)
            if len(errors) < 8:
    ...
    # Ranking
    # EXECUTAR: prioritize liquidity and clean trigger proximity
    results_exec.sort(key=lambda x: (-x[3], x[4]))  # dv20 desc, bbz (more negative better)
    # AGUARDAR: prioritize stronger compression and liquidity
    results_watch.sort(key=lambda x: (x[4], -x[3]))  # bbz asc (more negative), dv20 desc
    # NEAR: also by compression and liquidity
    ...
    msg.append("")
    msg.append(
        f"DIAG: hist_ok={n_hist_ok} | liq_ok={n_liq_ok} | comp_ok={n_comp_ok} | base_ok={n_base_ok} | dry_ok={n_dry_ok} | pipeline_ok={len(results_exec)+len(results_watch)}"
    )
    msg.append("")
    ...
    if not exec_top:
        ...
        for t, decision, px, dv, bbz, atrp, win, highb, dd, contr, vratio in exec_top:
            msg.append(
                f"- {t} | close={px:.2f} | dv20=${dv/1e6:.1f}M | base={win}d dd={dd:.2f} | trigger>={highb:.2f}"
            )
    ...
    if not watch_top:
        ...
        for t, decision, px, dv, bbz, atrp, win, highb, dd, contr, vratio in watch_top:
            msg.append(
                f"- {t} | close={px:.2f} | dv20=${dv/1e6:.1f}M | BBz={bbz:.2f} ATRpctl={atrp:.2f} | base={win}d dd={dd:.2f} | trigger>={highb:.2f}"
            )

    # Always show QUASE so you never get a “blind” FLAT day
    msg.append("")
    if near_top:
    ...
    if not exec_top and not watch_top:
        msg.append("")
        msg.append("FLAT: nenhum candidato final hoje (ver QUASE + DIAG para saber onde está a morrer).")
